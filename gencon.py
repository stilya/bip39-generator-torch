...
def generate_phrases(words, num_words, total_combinations, use_cuda=False, is_random=False):
    """
    Generate BIP39 phrases and write them to console.
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python generate_phrases.py <use_cuda> <generation_type>")
        print("<use_cuda>: y/n (y to use GPU, n to use CPU)")
        print("<generation_type>: r/a (r for random, a for alphabetical)")
        sys.exit(1)

    use_cuda_arg = sys.argv[1].lower()
    generation_type_arg = sys.argv[2].lower()

    if use_cuda_arg not in ['y', 'n'] or generation_type_arg not in ['r', 'a']:
        print("Invalid arguments. Please use 'y' or 'n' for CUDA and 'r' or 'a' for generation type.")
        sys.exit(1)
    ...
    use_cuda = cuda_available and use_cuda_arg == 'y'
    
    is_random = generation_type_arg == 'r'
    if is_random:
